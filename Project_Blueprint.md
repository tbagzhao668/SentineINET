...
## 5. 核心流程设计
1. **资产库**：用户维护设备资产（品牌/协议/地址/凭据/巡检间隔/备份服务器关联/备份间隔/备份开关/备份文件前缀）。
...
5. **配置备份（定期/手动）**：
   - 用户先配置备份服务器（TFTP/FTP/SFTP 等），并将设备关联到备份服务器。
   - 首次备份：通过 AI 生成“备份意图→命令模板”，执行成功后自动沉淀为备份 Skill（按品牌/版本/协议区分）。
   - 后续备份：直接复用已验证的备份 Skill，不再调用 AI。
6. **安全策略下发（可选）**：对高风险变更提供额外限制（配置开关/危险命令拦截/参数一致性校验），并记录策略历史。
...
- **备份命令治理**：定期备份优先走“已验证备份 Skill”，仅在首次学习或 Skill 不命中时调用 AI；AI 输出以命令模板形式落库，执行前渲染为真实命令，避免在仓库/日志中固化敏感信息。
